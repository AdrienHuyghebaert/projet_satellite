...
class Antenne:
    ...
    def creer_cercle(self, position_satellite):
        ...
        if np.all(point_intersection_cercle == np.zeros(3)):
            ...
            vect_direction_norm = vect_direction / np.linalg.norm(
                vect_direction)  # Vecteur direction normé entre le centre de la terre et du satellite
            ...
            pos_x = point_projete[0] + rayon_cercle * (
                        v1_plan_cercle[0] * np.cos(theta) + v2_plan_cercle[0] * np.sin(theta))
            pos_y = point_projete[1] + rayon_cercle * (
                        v1_plan_cercle[1] * np.cos(theta) + v2_plan_cercle[1] * np.sin(theta))
            pos_z = point_projete[2] + rayon_cercle * (
                        v1_plan_cercle[2] * np.cos(theta) + v2_plan_cercle[2] * np.sin(theta))
            ...
            vect_direction_norm = vect_direction / np.linalg.norm(
                vect_direction)  # Vecteur direction normé entre le centre de la terre et du satellite
            v1_plan_cercle = point_projete - point_intersection_cercle  # Vecteur appartenant au plan du cercle de
            # projection
            v1_plan_cercle = v1_plan_cercle / np.linalg.norm(v1_plan_cercle)  # On norme le vecteur
            v2_plan_cercle = np.cross(vect_direction_norm,
                                      v1_plan_cercle)  # Vecteur perpendiculaire qui permettra de créer le cercle
